...
const ProductCard = ({
  ...
  return (
    <article
      className={[
        "product-card group relative overflow-hidden rounded-[6px] bg-white shadow-[0_12px_30px_-24px_rgba(0,0,0,0.35)]",
        className,
      ]
        .filter(Boolean)
        .join(" ")}
    ...
    >
      {badge ? (
        <span
          className={`absolute left-4 top-4 z-10 rounded-[6px] px-3 py-1 text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-white ${
            badgeStyles?.[badge] ?? "bg-black"
          }`}
        >
          {badge}
        </span>
      ...
      <button
        ...
        aria-label={favoriteAriaLabel ?? `Save ${name}`}
      >
        {favoriteIcon}
      </button>
      ...
      <div className="relative overflow-hidden bg-[#f1f0ec]">
        <img
          ...
          loading="lazy"
          className={[
            "card-img-top w-full object-cover transition-transform duration-500 group-hover:scale-[1.04]",
            imageClassName ?? "h-[220px] min-[420px]:h-[260px] sm:h-[300px] lg:h-[360px]",
          ]
            .filter(Boolean)
            .join(" ")}
        />
        <Link
          to={href}
          className="absolute inset-x-0 bottom-0 translate-y-full bg-black/90 px-4 py-3 text-center text-sm text-white transition-transform duration-300 group-hover:translate-y-0"
        >
