...
import type { AuthSessionUser, ProfileEditFormProps, ProfileFormValues } from "../../../Types";
import { ProfileSchema } from "../../../Utils/ValidationSchemas";
import { getInitials, getPhoneNumber } from "../Shared";
...
const ProfileEditForm = ({profile,userId,profilePhoto,isSaving,isUploadingImage,isDeletingImage,editUser,uploadImage,deleteUploadedImage,onClose,}: ProfileEditFormProps) => {
  ...
  return (
    <Formik<ProfileFormValues> enableReinitialize initialValues={{   firstName: profile?.firstName ?? "",   lastName: profile?.lastName ?? "",   email: profile?.email ?? "",   phoneNumber: getPhoneNumber(profile) === "Not available" ? "" : getPhoneNumber(profile),   profilePhoto, }} validationSchema={ProfileSchema} onSubmit={async (values, { setSubmitting, setStatus, resetForm }) => {
        setStatus(undefined);
        if (!userId) {
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <label className={`inline-flex cursor-pointer items-center gap-2 rounded-full border border-[#d9d9d9] px-4 py-2 text-sm font-semibold text-[#111111] transition hover:border-black ${   isUploadingImage ? "pointer-events-none opacity-70" : "" }`}>
                    <UploadOutlined /> {isUploadingImage ? "Uploading..." : "Upload Photo"} <input type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} />
                  </label>
                  {values.profilePhoto ? (<button type="button" onClick={handlePhotoRemove} disabled={isDeletingImage} className="rounded-full border border-[#d9d9d9] px-4 py-2 text-sm font-semibold text-[#111111] transition hover:border-black">{isDeletingImage ? "Removing..." : "Remove"}</button>) : null}
                </div>
              </div>
            </div>
