      ...
      <div className="mx-auto w-[92%] max-w-[1400px]">
        ...
        <div className="grid gap-6 sm:gap-7 md:grid-cols-2 xl:grid-cols-3">
          {BLOG_POSTS.map((post) => (
            <article
              key={post.slug}
              className="group rounded-[12px] bg-white transition hover:shadow-[0_10px_30px_rgba(0,0,0,0.08)]"
            >
              <Link to={getBlogDetailPath(post.slug)} className="block overflow-hidden rounded-[12px]">
                <img
                  src={post.image}
                  alt={post.title}
                  className="h-[220px] w-full object-cover transition-transform duration-500 group-hover:scale-[1.05] sm:h-[260px] lg:h-[300px]"
                  loading="lazy"
                />
              </Link>
              ...
              <div className="p-4">
                ...
                <h2 className="mt-1 text-[1rem] font-semibold leading-[1.4] text-[#111827] sm:text-[1.05rem]">
                  <Link to={getBlogDetailPath(post.slug)} className="transition hover:text-black">
                    {post.title}
                  </Link>
                </h2>
                ...
                <Link
                  to={getBlogDetailPath(post.slug)}
                  className="mt-3 inline-flex text-[0.9rem] font-medium text-[#111827] transition hover:text-black"
                >
                  {"Continue Reading ->"}
                </Link>
              </div>
