...
const ContactPage = () => {
  ...
  <div className="mx-auto w-[92%] max-w-[1260px] rounded-[12px] bg-white p-5 sm:p-8 lg:p-10 shadow-sm">
    ...
    <div className="grid gap-8 lg:grid-cols-[360px_minmax(0,1fr)] lg:gap-10">
      <aside className="space-y-6">
        <div className="rounded-[10px] border border-[#e5e9ef] p-5">
          ...
          <p className="mt-3 text-[0.95rem] leading-7 text-[#5f6774]">1354 Green Street Nashville Drive Dodge City, KS 67801 United States</p>
          <p className="mt-3 text-[0.95rem] text-[#1f2937]"><span className="font-semibold">Email:</span> dhananjaypreet@gmail.com</p>
        </div>
        <div className="rounded-[10px] border border-[#e5e9ef] p-5">
          <h2 className="text-[1.4rem] font-semibold text-[#ef4444]">Make a Call</h2>
          <p className="mt-3 text-[1rem] font-semibold text-[#0b0b0b]">Customer Care:</p>
          <p className="mt-1 text-[0.95rem] text-[#5f6774]">+91 458 753 6924</p>
          <p className="mt-3 text-[1rem] font-semibold text-[#0b0b0b]">Careers:</p>
          <p className="mt-1 text-[0.95rem] text-[#5f6774]">+91 965 784 23658</p>
        </div>
        <div className="rounded-[10px] border border-[#e5e9ef] p-5">
          <h2 className="text-[1.4rem] font-semibold text-[#ef4444]">Drop A Mail</h2>
          <p className="mt-3 text-[0.95rem] leading-7 text-[#5f6774]">Fill out our form and we will contact you within 24 hours.</p>
          <p className="mt-3 text-[0.95rem] text-[#111827]">dhananjaypreet@gmail.com</p>
        </div>
      </aside>

      <div>
        <Formik<ContactFormValues> initialValues={{ name: "", email: "", mobileNumber: "", subject: "", message: "" }} validationSchema={ContactSchema} onSubmit={async (values, { setSubmitting, setStatus, resetForm }) => {   setStatus(undefined);
            try {
        ...
        >
          {({ isSubmitting, status }) => (
            <Form className="grid gap-4 sm:gap-5">
              ...
              {status?.error && (<p className="rounded-[10px] bg-[#ffecec] px-4 py-3 text-sm text-[#e53935]">{status.error}</p>)}
              {status?.success && (<p className="rounded-[10px] bg-[#ecfff0] px-4 py-3 text-sm text-[#1b7f3a]">  {status.success}</p>)}
              <button type="submit" disabled={isSubmitting || contactMutation.isPending} className="mt-2 inline-flex h-[50px] items-center justify-center rounded-[4px]  bg-[#111111] px-7 text-[0.95rem] font-semibold text-white transition  hover:bg-black disabled:cursor-not-allowed disabled:opacity-70 sm:w-fit">{isSubmitting || contactMutation.isPending ? "Sending..." : "Send Message"}</button>
            </Form>
          )}
        </Formik>
      </div>
