        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_300px]">
          <article>
            <div className="overflow-hidden rounded-[12px]">
              <img src={post.image} alt={post.title} className="h-[220px] w-full object-cover sm:h-[300px] lg:h-[380px]"/>
            </div>
            ...
            <h1 className="mt-2 text-xl font-semibold text-[#111827] sm:text-2xl lg:text-[1.7rem]">{post.title}</h1>
            <p className="mt-3 text-[0.95rem] leading-7 text-[#6b7280]">{post.excerpt}</p>
          ...
          <aside className="rounded-xl border border-[#e5e7eb] bg-[#fafafa] p-4 lg:sticky lg:top-[100px]">
            <p className="text-[0.7rem] font-semibold uppercase tracking-[0.15em] text-[#9aa4b2]">Latest Posts</p>

            <div className="mt-3 space-y-3">
              {relatedPosts.map((item) => (
                <Link key={item.slug} to={getBlogDetailPath(item.slug)} className="block rounded-lg bg-white p-3 transition hover:shadow-sm">
                  <p className="text-[0.7rem] text-[#9aa4b2]">{item.date}</p>
                  <p className="mt-1 text-[0.85rem] font-medium text-[#111827]">{item.title}</p>
                </Link>
              ))}
            </div>
          </aside>
